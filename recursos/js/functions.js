...
const abrirProcedimientos = () => {
	ValidarSesion('#contenido', 'procedimientos.html', '.menu', '#li-procedimientos');
};
const abrirReportes = () => {
...
function ObtenerDatosPaciente() {
	...
	$.ajax({
		...
		if (respuesta == 'NO REGISTRADO') {
			...
			$('#NombrePacienteC').val(paciente[0].apellidos + ', ' + paciente[0].nombre);
...
function validar_menorPaciente() {
	...
	if (document.getElementById('menorPaciente').checked) desbloquearControlsPaciente();
...
function RegistrarEstablecimiento() {
	...
	let data = { accion: 'REGISTRAR_ESTABLECIMIENTO', establecimiento: establecimiento };
	let llamadoAjax = ajaxFunction(data);
	alert(llamadoAjax);
...
function MostrarPagar() {
	...
	let json = JSON.parse(llamadoAjax);
	let cita = json.cita;
	$('#NombrePago').val(cita[0].apellidospaciente + ', ' + cita[0].nombrepaciente);
	$('#MotivoPago').val(cita[0].motivo);

	if (cita[0].estado === 'A CUENTA') MostrarPagarCuenta(cita[0].precio_consulta);
	else {
...
$(function() {
	$(document).on('click', '#tbCitas .fa-sliders-h', function(event) {
		...
		$.ajax({
			...
			$('#NombrePacienteC').val(cita[0].apellidospaciente + ', ' + cita[0].nombrepaciente);
...
function Reporte() {
	...
	let data = { accion: 'LISTAR_REPORTE', tipomovimientocaja: tipomov, fecha1: fecha1, fecha2: fecha2 };
	let llamadoAjax = ajaxFunction(data);
	...
	$('#tbReporte tr').each(function() {
		...
		if (tipopago === 'TRANSFERENCIA' && estado !== 'ANULADO') transferencia += monto;
...
function ListarCitasExternas() {
	...
	let data = { accion: 'LISTAR_CITAS_EXTERNAS', fecha1: fecha1, fecha2: fecha2, establecimiento: establecimiento };
	let llamadoAjax = ajaxFunction(data);
...
function RegistrarMovimientoA() {
	...
	} else if (producto == '' || cantidad == '' || cantidad < 1 || descripcion == '') {
